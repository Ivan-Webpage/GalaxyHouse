...
6. **內文**（支援多行）：
   - 直接貼文字或 HTML（例如 `<p>...</p>`、`<h2>...</h2>`、`<ul><li>...</li></ul>`），可以貼多行，**貼完之後另起一行輸入 `END` 再按 Enter，才會結束輸入**——這一步不能省略，否則工具只會存到你按下第一次 Enter 前的內容，後面貼的東西會消失
   - 或是先把內文寫在一個文字檔裡，第一行直接輸入 `@檔案路徑` 讓工具讀檔案內容進來（不用再輸入 `END`，內文比較長、需要仔細排版時建議用這個方式）
